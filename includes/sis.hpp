...
/// has to be less than N.
  ...
  /// \brief Takes input Lmat, Mmat and number of eigenvalues to be computed.
  /// This
  /// computes eigenvalues based on a reduced matrix when Mmat is singular.
  ///
  /// For example, the problem \f[
  /// \partial_{yyyy} v(y)\;=\;  \lambda\, \partial_{yy} v(y)
  /// \f]
  /// with boundary conditions \f$ v(\pm 1) \;=\; v'(\pm 1)  \;=\; 0\f$ can be
  /// shown to be linearly stable. However the native implementation of this
  /// problem can produce spurious modes. These modes are filtered out. To see
  /// the spuroius modes, call the compute compute function to display all the
  /// (N+1) eigenvalues.
  ///
  /// The solution to this problem is to reformulate the problem in the
  /// following manner:
  ...
  /// \right]
  /// \f]
  /// with boundary conditions on \f$v(y)\f$, \f$ v(\pm 1) \;=\; v'(\pm 1) \;=\;
  /// 0\f$ and no boundary conditions on \f$\psi(y)\f$. However, solving the
  /// generalized eigenvalue also brings infinite eigenvalues which can also be
  /// undesirable. For this reason, we have compute_reduced. This will apply
  /// a series of transformations to reduce the whole descriptor system to a
  /// state-space.
  ///
  /// We provide this functionality as many problems in Fluid dynamics involve
  /// operators with the above structure.
  void compute_reduced(const LinopMat<T> &Lmat_, const LinopMat<T> &Mmat_,
                       int num_vals) {
    LinopMat<T> Lmat = Lmat_;
    LinopMat<T> Mmat = Mmat_;
    ...
    for (int i = 0; i < c; i++) {
      ...
      highest_each_column[i] =
          (highest_each_columnL[i] > highest_each_columnM[i])
              ? highest_each_columnL[i]
              : highest_each_columnM[i];
      //  std::cout << "highC["<< i << "] = " << highest_each_column[i] << '\n';
    }
    for (int i = 0; i < c; i++) {
      total_boundary_conditions += Lmat.BcVec[i].nbc();
      num_bc_each_var[i] = Lmat.BcVec[i].nbc();
      //  std::cout << "numbcevar["<< i << "] = " << num_bc_each_var[i] << '\n';
    }
    ...
    // Declare the master matrix L:
    Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> masterL(r * (N + 1),
                                                             c * (N + 1));
    // Declare the master matrix M:
    Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> masterM(r * (N + 1),
                                                             c * (N + 1));
    ...
    masterL = masterL.eval() + (subs_mat_col_mul * subs_mat);
    masterM = masterM.eval() + (subs_mat_col_mul2 * subs_mat);
    Eigen::ColPivHouseholderQR<
        Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> >
        solver;
    ...
    QRrecursive<T> reducer(masterM);
    reducer.compute_eigen_system(masterL, masterM);
    bool Is_M_Invertible = true;
    Eigen::Matrix<std::complex<T>, Eigen::Dynamic, Eigen::Dynamic>
        evecMat_master(c * (N + 1), reducer.evecs.cols());
    Eigen::Matrix<std::complex<T>, Eigen::Dynamic, 1> temp_vec;
    Eigen::Matrix<std::complex<T>, Eigen::Dynamic, Eigen::Dynamic> fir_n_mat =
        subs_mat * reducer.evecs;
    if (num_vals > reducer.evecs.cols()) {
      std::cout << "You've requested " << num_vals
                << " eigenvalues, while only " << reducer.evecs.cols()
                << " are available after reduction. Only "
                << reducer.evecs.cols()
                << " eigenvalues and corresponding eigenvectors will be stored."
                << '\n';
      num_vals = reducer.evecs.cols();
    }
    row_counter = 0;
    col_counter = 0;
    for (int i = 0; i < c; i++) {
      ...
      Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> evecMat(
          N + 1 + n, reducer.evecs.cols());
      evecMat.setConstant(0.0);
      evecMat.block(Lmat.BcVec[i].nbc(), 0, N + 1 + n - Lmat.BcVec[i].nbc(),
                    reducer.evecs.cols()) =
          reducer.evecs.block(row_counter, 0, N + 1 + n - Lmat.BcVec[i].nbc(),
                              reducer.evecs.cols());
      row_counter += N + 1 + n - Lmat.BcVec[i].nbc();
      evecMat.block(0, 0, Lmat.BcVec[i].nbc(), reducer.evecs.cols()) =
          fir_n_mat.block(col_counter, 0, Lmat.BcVec[i].nbc(),
                          reducer.evecs.cols());
      col_counter += Lmat.BcVec[i].nbc();
      evecMat_master.block(i * (N + 1), 0, N + 1, reducer.evecs.cols()) =
          Mat.mats2[n] * evecMat;
    }
    std::vector<std::vector<T> > eigenval_trunc_sorter;

    eigenval_trunc_sorter.resize(reducer.evecs.cols());
    for (int i = 0; i < reducer.evecs.cols(); i++) {
      eigenval_trunc_sorter[i].resize(2);
    }

    Eigen::Matrix<int, Eigen::Dynamic, 1> MPcount(reducer.evecs.cols());
    MPcount.setConstant(0);
    std::vector<EigenSorter<T> > sorter;
    sorter.resize(reducer.evecs.cols());
    for (int i = 0; i < reducer.evecs.cols(); i++) {
      sorter[i].compute(evecMat_master.block(0, i, c * (N + 1), 1), c);
      if (!Is_M_Invertible) {
        // If M is not invertible, there will be zero eigenvalues, so that 1/0 =
        // inf eigenvalue, remove them.
        if (abs(reducer.evals[i]) < 1e-11) {
          sorter[i].numMp = N + 1;
          sorter[i].Mp = false;
          sorter[i].minVal = 1e11;
        }
    ...
    if (MPcount_sum >= num_vals) {
      ...
      for (int i = 0; i < num_vals; i++) {
        if (Is_M_Invertible) {
          eigenvalues[i] = reducer.evals[int(eigenval_trunc_sorter[i][1])];
#ifdef SIS_USE_LAPACK
          alpha[i] = reducer.alpha[int(eigenval_trunc_sorter[i][1])];
          beta[i] = reducer.beta[int(eigenval_trunc_sorter[i][1])];
#endif
        } else {
          eigenvalues[i] =
              1.0 / reducer.evals[int(eigenval_trunc_sorter[i][1])];
        }
      }
      for (int j = 0; j < c; j++) {
        int n = (highest_each_columnL[j] >= highest_each_columnM[j])
      ...
      std::vector<std::vector<T> > eigenval_trunc_sorter2;
      eigenval_trunc_sorter2.resize(reducer.evecs.cols() - MPcount_sum);
      for (int i = MPcount_sum; i < reducer.evecs.cols(); i++) {
        eigenval_trunc_sorter2[i - MPcount_sum].resize(2);
        eigenval_trunc_sorter2[i - MPcount_sum][0] =
            sorter[int(eigenval_trunc_sorter[i][1])].minVal;
        eigenval_trunc_sorter2[i - MPcount_sum][1] =
      ...
      for (int i = MPcount_sum; i < num_vals; i++) {
        if (Is_M_Invertible) {
          eigenvalues[i] =
              reducer.evals[int(eigenval_trunc_sorter2[i - MPcount_sum][1])];
#ifdef SIS_USE_LAPACK
          alpha[i] =
              reducer.alpha[int(eigenval_trunc_sorter2[i - MPcount_sum][1])];
          beta[i] =
              reducer.beta[int(eigenval_trunc_sorter2[i - MPcount_sum][1])];
#endif
        } else {
          eigenvalues[i] =
              1.0 /
  ...
  /// Use this to solve boundary constraits to Eigenvalues in reduced form. This
  /// means there are some boundary conditions that are constrained by the eigen
  /// value. Again solvability must hold. That total of all the highest orders
  /// of every independent variable has to be equal to the total number of
  /// boundary conditions specified + the number of constraints. Further, please
  /// ensure that the boundary conditions and constraints are linearly
  /// independent.
  ///
  /// This does the same as compute with constraints, but brings the matrices to
  /// the reduced form, that is, it removes infinite eigenvalues.
  /// See compute_reduced() for more details.
  void compute_with_constraints_reduced(const LinopMat<T> &Lmat_,
                                        const LinopMat<T> &Mmat_, int num_vals,
                                        const BcMat<T> &Lbc_,
                                        const BcMat<T> &Mbc_) {
    ...
    /*std::cout <<"masterL: " <<  size(masterL) << '\n';
    std::cout << "msaterM: " << size(masterM) <<  '\n';
    std::cout << "constraints_bigM: " << size(constraints_bigM) << '\n';
    std::cout << "constraints_smallM: " << size(constraints_smallM) << '\n';
    std::cout << "subs_mat: " << size(subs_mat) << '\n';
    std::cin >> bre;
*/
    if (mat_temp.rows() != 0 || mat_temp.cols() != 0) {
      masterL.block(c * (N + 1), 0, Lbc.m, c * (N + 1) + Lbc.m) =
